...
if (SAR_KILL_MSG) then {
    ...
    if(isPlayer _aikiller) then {
        _message = format["Un %3 %2 fue asesinado por: %1",_aikiller_name,_ai_type,_ai_xp_type];
        diag_log _message;
    ...
    if(!isPlayer _aikiller && !(isNull _aikiller)) then {
        ...
        if (_ai_xp >=SAR_AI_XP_LVL_2) then {
            _message = format["Un %1 %2 fue asesinado por %3 %4!",_ai_xp_type,_ai_type,_ai_killer_xp_type,_ai_killer_type];
            diag_log _message;
            [nil, nil, rspawn, [[West,"airbase"], _message], { (_this select 0) sideChat (_this select 1) }] call RE;
        };
    };
...
if(isPlayer _aikiller) then {
    ...
    if (_aikilled_group_side == SAR_AI_friendly_side) then {
        ...
        if ((random 100) < 3) then {
            _message = format["%1 ha asesinado a un compañero - enviando refuerzos!",_aikiller_name];
            [nil, nil, rspawn, [[West,"airbase"], _message], { (_this select 0) sideChat (_this select 1) }] call RE;
        } else {
            if ((random 100) < 3) then {
                _message = format["Tango down ... ofrecemos una gran recompensa por la cabeza de %1!",_aikiller_name];
                [nil, nil, rspawn, [[West,"airbase"], _message], { (_this select 0) sideChat (_this select 1) }] call RE;
            };
        };
        if ((random 100) < 2) then {
            _message = "NADA DE MAYUSCULAS EN EL CHAT !!!";
            [nil, nil, rspawn, [[West,"airbase"], _message], { (_this select 0) sideChat (_this select 1) }] call RE;
        };

    };
    if (_aikilled_group_side == SAR_AI_unfriendly_side) then {
        if(SAR_DEBUG)then{diag_log format["SAR_DEBUG: Ajustando humanidad para bandido asesinado por %2 de %1",_aikiller,SAR_band_kill_value];};
        _humanity = _aikiller getVariable ["humanity",0];
        ...
        if ((random 100) < 3) then {
            _message = format["%1 es el amo!",_aikiller_name];
            [nil, nil, rspawn, [[West,"airbase"], _message], { (_this select 0) sideChat (_this select 1) }] call RE;
        } else {
            if ((random 100) < 3) then {
                _message = format["otro que cae ... %1 se los va a vendimiar a todos!! :-)",_aikiller_name];
                [nil, nil, rspawn, [[West,"airbase"], _message], { (_this select 0) sideChat (_this select 1) }] call RE;
            };
        };
        if ((random 100) < 2) then {
        ...
        if(!isNull _aikiller) then { // check if AI was killed by an AI, and not driven over / fallen to death etc
            ...
            if(_ai_killer_xp < SAR_AI_XP_LVL_3) then {
                ...
                if(_ai_killer_xp < SAR_AI_XP_LVL_2 && _ai_killer_xp_new >= SAR_AI_XP_LVL_2 ) then { // from level 1 to level 2
                    ...
                    if(SAR_SHOW_XP_LVL) then { diag_log format["%1 sube de nivel 1 -> 2",_aikiller];};

                    _message = format["Un %1 %2 fue promocionado!",_ai_killer_xp_type,_ai_killer_type];
                    [nil, nil, rspawn, [[West,"airbase"], _message], { (_this select 0) sideChat (_this select 1) }] call RE;
                ...
                if(_ai_killer_xp < SAR_AI_XP_LVL_3 && _ai_killer_xp_new >= SAR_AI_XP_LVL_3 ) then { // from level 2 to level 3
                    ...
                    if(SAR_SHOW_XP_LVL) then { diag_log format["%1 sube de nivel 2 -> 3",_aikiller];};
