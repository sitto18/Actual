...
// --------------------------------------------------
    ...
    // TOPOLKA, 2 bandit groups, 1 soldier groups, 3 survivor groups - spawn probability ba,so,su - maximum group members ba,so,su
    _check = [["max_grps","rnd_grps","max_p_grp"],[[2,1,3],[75,50,75],[4,0,4]],"SAR_area_2_0"] call SAR_AI_mon_upd; 

    // SEVEN, 1 bandit groups, 1 soldier groups, 3 survivor groups - spawn probability ba,so,su - maximum group members ba,so,su
    _check = [["max_grps","rnd_grps","max_p_grp"],[[1,1,3],[50,20,50],[4,0,2]],"SAR_area_3_0"] call SAR_AI_mon_upd; 
...
//
	...
	[SAR_marker_car_bandBranibor,1,["SUV_Special"],[[1,2,1]],true,60] call SAR_AI_land;
	
	// Bandit Car Patrol Airport
	[SAR_marker_car_bandAirport,1,["UralCivil2"],[[1,3,4]],true,60] call SAR_AI_land;
