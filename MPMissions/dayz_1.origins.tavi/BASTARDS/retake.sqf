...
if(_nearby > 1) exitWith { cutText [format["No se puede retirar %1 de %2, hay otro jugador demasiado cerca.",_namePart,_nameType], "PLAIN DOWN"]; };
...
if (_hasToolbox && (_part != "PartGlass")) then {
	...
	cutText [format["Necesitas una toolbox para quitar %1",_namePart], "PLAIN DOWN"];
};